...
public class onTrigger2DUntil: MonoBehaviour
{
    ...
    public Vector2 DirectionToTarget => target.trasnform.position - detectorOrigin.position;
    ...
    private void Start() {
        StartCoroitine(DetectionCoroutine);
    }
    ...
    public void PerformDetection(){
        Collider2D collider = Physics2D.OverlayBox(
    ...
    private void OnDrawGizmos() {
        if (showGizmos && detectorOrigin != null) {
            ...
            if(PlayerDetected){
                Gizmos.color = dizmoDetectedColor;
            }
